...
def create_client_thread(client, pedido):
    t = threading.Thread(target=handle_client, args=(client, pedido))
    t.start()
    return t

def handle_new_client(client, pedido):
    t = create_client_thread(client, pedido)
    t.join()

if __name__ == "__main__":
    # Aquí es donde aceptarías nuevos clientes y pedidos.
    # Por ahora, solo estoy usando un cliente y pedido de ejemplo.
    ...
    handle_new_client(client, pedido)
"""
